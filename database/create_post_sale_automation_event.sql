...
BEGIN
    ...
    INNER JOIN (
        SELECT
            TRIM(name) COLLATE utf8mb4_unicode_ci AS stage_name,
        ...
        ON ps.stage_name = TRIM(p.status) COLLATE utf8mb4_unicode_ci
    LEFT JOIN pos_venda_stages pvst
        ON pvst.id = ps.post_sale_target_stage_id
    LEFT JOIN pos_venda pv
        ON pv.project_id = p.id
    WHERE COALESCE(ps.post_sale_enabled, 0) = 1
      AND pv.id IS NULL
      AND COALESCE(p.moved_to_post_sale, 0) = 0
      AND DATEDIFF(
            CURDATE(),
